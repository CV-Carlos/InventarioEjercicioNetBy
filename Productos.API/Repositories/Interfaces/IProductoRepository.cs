...
namespace Productos.API.Repositories.Interfaces
{
    ...
    {
        Task<PaginadoDto<Producto>> ObtenerTodosAsync(string? nombre, int? categoriaId, decimal? precioMin, decimal? precioMax, bool? activo, int pagina, int itemsPorPagina);
        Task<Producto?> ObtenerPorIdAsync(int id);
        Task<Producto> CrearAsync(Producto producto);
        Task<Producto> ActualizarAsync(Producto producto);
        Task<bool> EliminarAsync(int id);
        Task<bool> ActualizarStockAsync(int id, int cantidad);
        Task<IEnumerable<Producto>> ObtenerParaSelectAsync();
    }
}